...
SET LAMBDA_NAME=Todo2024_DeletePerson_ECR
SET IMAGE_NAME=delete_person
SET VERSION=1.0
...
aws lambda add-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE% --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn "arn:aws:execute-api:%REGION%:%ACCTNUM%:%APIHASH%/*/%REQUEST_TYPE%/person/{id}"
