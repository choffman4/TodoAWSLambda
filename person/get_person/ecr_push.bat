...
aws lambda remove-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE%_person

REM nasty - doesn't like trailing / on person
REM aws lambda add-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE%_person --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn "arn:aws:execute-api:us-east-2:215330149056:k404zm8112/*/%REQUEST_TYPE%/person"
aws lambda add-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE%_person --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn "arn:aws:execute-api:%REGION%:%ACCTNUM%:%APIHASH%/*/%REQUEST_TYPE%/person"

aws lambda remove-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE%_personid
REM aws lambda add-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE%_personid --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn "arn:aws:execute-api:us-east-2:215330149056:k404zm8112/*/%REQUEST_TYPE%/person/{id}"
aws lambda add-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE%_personid --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn "arn:aws:execute-api:%REGION%:%ACCTNUM%:%APIHASH%/*/%REQUEST_TYPE%/person/{id}"
