...
SET LAMBDA_NAME=Todo2024_GetTodo_ECR
SET IMAGE_NAME=get_todo
SET VERSION=1.0
...
aws lambda remove-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE%_todo

REM nasty - doesn't like trailing / on todo
REM aws lambda add-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE%_todo --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn "arn:aws:execute-api:us-east-2:215330149056:k404zm8112/*/%REQUEST_TYPE%/todo"
aws lambda add-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE%_todo --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn "arn:aws:execute-api:%REGION%:%ACCTNUM%:%APIHASH%/*/%REQUEST_TYPE%/todo"

aws lambda remove-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE%_todoid
REM aws lambda add-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE%_todoid --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn "arn:aws:execute-api:us-east-2:215330149056:k404zm8112/*/%REQUEST_TYPE%/todo/{id}"
aws lambda add-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE%_todoid --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn "arn:aws:execute-api:%REGION%:%ACCTNUM%:%APIHASH%/*/%REQUEST_TYPE%/todo/{id}"
