...
SET LAMBDA_NAME=Todo2024_DeleteTodo_ECR
SET IMAGE_NAME=delete_todo
SET VERSION=1.0
...
REM aws lambda add-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE% --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn "arn:aws:execute-api:us-east-2:215330149056:k404zm8112/*/%REQUEST_TYPE%/todo/{id}"
aws lambda add-permission --function-name %LAMBDA_NAME% --statement-id %LAMBDA_NAME%-%REQUEST_TYPE% --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn "arn:aws:execute-api:%REGION%:%ACCTNUM%:%APIHASH%/*/%REQUEST_TYPE%/todo/{id}"
